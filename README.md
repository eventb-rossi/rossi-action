...
Security tab. The same validation pass supplies the SARIF file, annotations and
counts.
...
| `version` | latest | rossi version to install, without the leading `v`. Must be v0.1.8 or newer. |
...
**rossi v0.1.8 or newer.** That is the release whose structured reports carry
the joined member path, keep rule-less failures in SARIF, and itemise a
directory holding no components — the action reads all three, and refuses an
older binary rather than miscount. The default `version:` (latest) satisfies
this; only an explicit `version:` or `rossi-path:` can fall below it.

On the runner: `bash`, `curl` and `jq` — all present on GitHub-hosted runners
for Linux, macOS and Windows. Prebuilt rossi binaries exist for x86-64 and
arm64 on all three.
